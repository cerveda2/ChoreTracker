...
2. ~~**Real-time Firestore sync**~~ — Done. `RemoteHouseholdDataSource.observeMembers`/`observeCompletions` (Firebase `addSnapshotListener`, `core/remote-firebase`) feed a reactive subscription in `LocalSyncRepository` (`core/sync`) that starts once a user + household are known and writes straight into Room — no UI/ViewModel changes needed since reads were already Flow-driven. Chores/invites remain pull-only (same pattern can extend later). Fixes both the lagging-completion bug and the "owner doesn't see accepted invite until cold start" issue.
